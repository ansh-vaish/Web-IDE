...
const WebContainerPreview = ({
  ...
  const getRunCommandFromPackageJson = async (
    container: WebContainer,
  ): Promise<[string, string[]]> => {
    const packageJsonRaw = await container.fs.readFile("package.json", "utf8");
    ...
    throw new Error(
      "No runnable script found. Expected package.json scripts.dev or scripts.start",
    );
  };
  ...
  useEffect(() => {
    async function setupContainer() {
      ...
      try {
        ...
        const [command, args] = await getRunCommandFromPackageJson(instance);
        if (terminalRef.current?.writeToTerminal) {
          terminalRef.current.writeToTerminal(
        ...
        const startProcess = await instance.spawn(command, args);
